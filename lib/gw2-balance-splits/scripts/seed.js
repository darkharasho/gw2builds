...
function parseInfoboxParams(wikitext) {
  const facts = [];
  const pattern = /\|\s*(\w[\w\s]*?)\s+wvw\s*=\s*([^\n|]+)/g;
  let match;
...
async function extractFromWikitext(pageName) {
  ...
  try {
    ...
    if (hasWvwSpecific) {
      ...
      facts = parseInfoboxParams(wikitext);
    }
...
module.exports = { parseWikitextFacts, mapWikiFactToApiFact, parseInfoboxParams, validateSplitEntry };
