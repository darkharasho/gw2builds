...
function mapWikiFactToApiFact(factType, positional, params, isWvw, isUniversal) {
  ...
  switch (factType) {
    ...
    case "radius": {
      ...
      // Named radius (alt= param) → type "Number" to match GW2 API convention for named radii
      if (params.alt) {
        return { type: "Number", text: stripWikiMarkup(params.alt), value: dist, icon: RADIUS_ICON };
      }
      return { type: "Radius", text: "Radius", distance: dist, icon: RADIUS_ICON };
    }
    case "blast radius":
    case "healing radius":
    case "barrier radius":
